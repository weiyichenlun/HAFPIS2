 ...
 * 最后修改时间:2018/5/8
 */
public class FourPalmMasterDaoJDBC implements MasterDAO, Serializable {
    private Logger log = LoggerFactory.getLogger(FourPalmMasterDaoJDBC.class);
    ...
    public void finishRecord(String id) {
        ...
        try {
            this.queryRunner.update(sql, Record.Status.Done.name(), id, CONSTANTS.RECORD_DATATYPE_PP,
    ...
    public void resetRecordPublishing(String magic) {
        ...
        try {
            this.queryRunner.update(publicshSql, Record.Status.Trained.name(), null, Record.Status.Publishing.name(),
                    CONSTANTS.RECORD_DATATYPE_PP, magic);
        } catch (SQLException e) {
    ...
    public void listId(Function2<? super String, ? super Date, Boolean> block) {
        ...
        try {
            ...
            res.forEach(objects -> {
                ...
                if (imgMask != null) {
                    if (imgMask.length() >= 30 && !imgMask.substring(20, 30).equals("0000000000")) {
